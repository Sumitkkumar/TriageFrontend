...
const GetOrder = () => {
  ...
  const fetchAdditionalData = async (dataflowName, conversationId) => {
    const additionalDataUrl = "http://localhost:8080/api/order/warehouse-to-rapi";
  ...
  const displayMoreData = async (key) => {
    ...
    let additionalData = await fetchAdditionalData(dataflowName, conversationId);
    setWarehouseToRapiData(additionalData.substring(17));
  };
